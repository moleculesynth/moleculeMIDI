// UPDATED by Molecule Synth for Arduino 1.0+
// http://github.com/moleculesynth
//
/*  Midi.cpp: Code for MIDI processing library, Arduino version 1.0+
 *            
 *  This file is a revision of part of Tymm's Arduino Midi Library.
...
void Midi::sendNoteOff(unsigned int channel, unsigned int note, unsigned int velocity)
{
    ...
    serial_.write(note & 0x7f);
    serial_.write(velocity & 0x7f);
}
...
void Midi::sendControlChange(unsigned int channel, unsigned int controller, unsigned int value)
{
    ...
    serial_.write(controller & 0x7f);
    serial_.write(value & 0x7f);
}
...
{
    serial_.write(STATUS_PITCH_CHANGE);
    serial_.write(pitch & 0x7f);
    serial_.write((pitch >> 7) & 0x7f);
}
...
{
    serial_.write(STATUS_SONG_POSITION);
    serial_.write(position & 0x7f);
    serial_.write((position >> 7) & 0x7f);
}
...
{
    serial_.write(STATUS_SONG_SELECT);
    serial_.write(song & 0x7f);
}
...
{
    serial_.write(STATUS_TUNE_REQUEST);
}
...
{
    serial_.write(STATUS_ACTIVE_SENSE);
}
